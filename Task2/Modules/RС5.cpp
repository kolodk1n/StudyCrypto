...
RC5_Cryptor::RC5_Cryptor(const string& Input, const string& Output, const string& Pass)
{
  FileIn = Input;
  FileOut = Output;
  psw = Pass;
...
bool RC5_Cryptor::RC5_Encrypt ()
{
  ...
  pull.write((char*)iv, RC5::BLOCKSIZE);
  pull.close();
...
bool RC5_Cryptor::RC5_Decrypt ()
{
  ...
  if (pool.good())
  {
    pool.read((char*)&iv, RC5::BLOCKSIZE);
    pool.close();
  }
