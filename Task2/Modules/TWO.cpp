...
Twofish_Cryptor::Twofish_Cryptor(const string& Input, const string& Output, const string& Pass)
{
  FileIn = Input;
  FileOut = Output;
  psw = Pass;
...
bool Twofish_Cryptor::Twofish_Encrypt ()
{
  ...
  pull.write((char*)iv, Twofish::BLOCKSIZE);
  pull.close();
  ...
  try
  {
    CBC_Mode<Twofish>::Encryption encr;
...
bool Twofish_Cryptor::Twofish_Decrypt ()
{
  SecByteBlock key(Twofish::DEFAULT_KEYLENGTH);
  PKCS12_PBKDF<SHA512> pbkdf;
  pbkdf.DeriveKey(key.data(), key.size(), 0, (byte*)psw.data(), psw.size(), (byte*)salt.data(), salt.size(), 1024, 0.0f);
  ...
  if (pool.good())
  {
    pool.read((char*)&iv, Twofish::BLOCKSIZE);
    pool.close();
  }
  ...
  try
  {
    CBC_Mode<Twofish>::Decryption decr;
